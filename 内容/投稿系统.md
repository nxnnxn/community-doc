...
1. MySql一般原则,单表百万量级合适,单库百表量级别合适,这样数量级我们需要进行分库分表，若单表500W条,单库100张表记录,数量上大概有5亿,存储上大概有500G,整体比较合适。
1. 拆分规则:整体拆分我们一般按照2的倍数来拆分,因此则按照用户ID取模来拆分,分为128库/128张表，500亿/128/128=305W条，一张表大概3G，128张表大概370G左右.
1. 成本:由于数据比较热的一般是10%的数据,所以数据库可以选32G,CPU选16核,基本单库支撑1WQPS不是问题,价格打个5折一般在1800左右,128库.价格大概在每月23W左右,1年成本大概=276W
...
1. 存储介质:业务上我们需要对内容数据进行缓存,以提供最基本的byids查询,因此选择内存型.
