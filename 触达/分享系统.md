...
1. 在大多数产品的分享功能中可以集成非常多的SDK,如分享到微信,分享到微博,分想到抖音等等.
2. 现阶段我们只做最基本分享链接,不集成各自的SDK,以防止被封杀，但可以通过生成图片+二维码的方式来分享到各个SDK
   1. 功能1:支持分享链接,链接里需要包含哪种用户把内容从那种设备通过哪种渠道分享了出去。
   2. 功能2:生成图片,且图片带上分享链接
   3. 功能3:分享的渠道支持微信,微博等渠道
   4. 功能4:要支持回流追踪
   5. 功能5:要
