...
在FEED分发中核心有三种内容的分发,分别为社交流,算法流和地理流

1. 社交流
   1. 社交流也就是基本的关注流，主要用于通过社交关系链来传播内容，用于培养用户对社区的感情，提高用户粘性，培养社区氛围。
   2. 社交分发另一大优势用于用户打破信息茧房，让你看到更多的信息
2. 算法流
   1. 算法流核心是标签匹配分发，主要是推荐系统所干之事
   2. 在初期需要运营划定精品内容，用于培养平台调性
   3. 需要运营干预能力，方便运营对重点内容进行更多曝光
3. 地理流
   1. 地理流一般会同城发布，在空间维度上完成信息的分发,也基本是算法流
   2. 推荐一般会考虑远近，热度等，同时可以用于推进自己的本地服务能力
...
![](https://www.plantuml.com/plantuml/proxy?src=https://raw.githubusercontent.com/nxnnxn/community-doc/main/puml/distribute/feed/system.puml&fmt=svg&cache=no)
...
读取流量预估,峰值QPS整体按照5倍预估

1. 首页算法流帖子FEED流量预估,2亿* 6(一天打开6次)* 3(平均下查3个请求)/3600/24=41666，峰值=平均*5=20.9WQPS
2. 用户在首页跳出到关注流是20%,20.9W*0.2=41666QPS
3. 用户在首页跳出到地理流是10%,20.9W*0.1=20999QPS
4. 整体QPS=20.9W+4.2W+2.1W+冗余=30W QPS
...
> 相关文档介绍https://help.aliyun.com/zh/tablestore/use-cases/design-a-feed-stream-system-that-supports-tens-of-millions-of-tps-and-qps

 关注流其实就是用户关注的用户所发的内容数据,一般每个用户都有收件箱和发件箱，前期一般以时间为顺序,但现有如抖音/淘宝等一般都是再rerank一下然后再分发出去。整体还是以推拉结合的模式比较多.

1. 推拉模式基本流程为当拉模式用户群发布内容的时候,只写用户自身的发件箱,推模式用户有很多的粉丝,比如千万级别粉丝,需要在

大体思路是区分大V,在关注列表的score中用于存储用户读取feed timestamp,根据用户发布的feedsocre来做一次过滤,再根据本地缓存哪些用户是活跃用户来完成check,最后再拉取数据,核心是不需要获取所有的用户,只需要获取关注列表中未同步的数据,在线用户数据都同步

1. 如何识别
   1. 通过天级别离线任务标识大V,然后通过应用任务来更新相关大V,同时大V不能回退
   2. 通过运营后台来让平台运营标识大V
...
3. 对于大V采用拉模式，读放大，生成feed列表。
4. 对于普通用户采用推模式，写放大，生成fed列表。
   X 1. 发布itme时，如果是大V则仅写入到自身的发件箱中
5. 发布item时，如果是普通用户，则进行写放大的推模式
6. 读取Feed时，读取关注列表，从中识别出哪些是大V用户，并行的读取自身的收件箱，2 然后读取其他大V的收件箱，拿到i t e m i d l i s t 后进行合并，返回最新的f e e d 数据。
   用户分级策略 对于大V用户
7. 通过粉丝数/离线通过舆情热度进行计算/ML模型打标等手段识别是否为大V，将大V作为一种用户 的标签属性进行存储。
   2.通过Flink流式的计算，来识别是否为大V发文。
8. 大V仅能升级而不能降级， 一旦降级将需要回溯所有粉丝的收件箱
   对于活跃用户
9. 基于月活/日活用户判断 一个用户是否为活跃用户甚至可以维护活跃级别
10. 一月内活跃为1级，收件箱保留长度100条，周活跃为2级，收件箱长度保留300条，前一天活跃为3 级，收件箱长度保留500条，用户平均一天刷N刷则为4级用户，保留1000条。
11. 大V用户也可以分级别，1000粉丝是1级，5000粉丝是2级，1000粉丝是3级等等，
    X 1. 不同级别的大V可以push的粉丝比例也可以不同，优先将Feed推送给活跃用户，以保证 其用户体验。
12. 但这种方式就造成在读取的时候，为每个用户需要独立维护 一个关系状态，用来判断该 大V是否应该给此用户推送Feed，将会变得很复杂。
    收益
    实现上最为简单 1.读的延迟不如纯写放大，但也要快于完全的读放大。
13. 写的延迟不如玩去的读放大，但也解决了LadyGaga 问题
    风险
    产品功能受限
14. 大V用户如何识别才能避免边界问题造成的性能抖动?
15. 大V发布fe e d ，所有人去拉大V的最新fe e d ，如果大V的发件箱所 在服务器宕机，将导致大V发布的Feed丢失。
16. 如果用户关注的用户过多，收件箱列表过大，会造成大Key问 题，将如何解决?
17. app的注册用户很多，但是活跃用户较少，如果为每个用户都存 储收件箱是否会占用太多存储成本?
18. 对于写放大的推模式，如果一个feed被删除，那么删除状态应 如何快速更新?
19. 收件箱如果写入失败将如何处理?
20. 对于收件箱，redis内存的使用将随着用户的增多而增大，运维 与机器成本不可忽略。
    给粉丝数加个上限
    推拉结合
    大v写放大不可行，可以降级为读放大模式。
21. 通过多种手段，可以避免大V的边界造成的性能抖动。
22. 通过细粒度的用户分级策略，可以在存储成本，读取延迟之 间作出较好的权衡。
23. 部分活跃用户的收件箱拥有大V的feed ，将减少对大V发件 箱的依赖，可靠性得到提升。
24. 仅对活跃用户进行推送，而不活跃用户将不会产生存储额外 的成本。
    该
